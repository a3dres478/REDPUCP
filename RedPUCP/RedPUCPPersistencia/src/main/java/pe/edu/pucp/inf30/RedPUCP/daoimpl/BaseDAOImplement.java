...
public abstract class BaseDAOImplement<T> implements ICrud<T>{
    ...
    public int insertar(T modelo) {
        try (
                Connection conn = DBManager.getInstance().getConnection(); CallableStatement cmd = this.comandoInsertar(conn, modelo);) {
    ...
    public boolean modificar(T modelo) {
        try (
                Connection conn = DBManager.getInstance().getConnection(); PreparedStatement ps = this.comandoModificar(conn, modelo);) {
    ...
    public boolean eliminar(int id) {
        try (
                Connection conn = DBManager.getInstance().getConnection(); PreparedStatement ps = this.comandoEliminar(conn, id);) {
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
    ...
    public T buscar(int id) {
        try (
                Connection conn = DBManager.getInstance().getConnection(); PreparedStatement ps = this.comandoBuscar(conn, id);) {
    ...
    public List<T> listar() {
        try (
                Connection conn = DBManager.getInstance().getConnection(); PreparedStatement ps = this.comandoListar(conn);) {
            ResultSet rs = ps.executeQuery();
