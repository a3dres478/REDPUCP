...
public class UsuarioComunResource {
    ...
    public boolean login(@FormParam("email") String email, 
        ...
        return comunBO.verificarLogin(email, password);
    }
    
    @GET
    @Path("correo/{correo}")
    public Response obtenerPorCorreo(@PathParam("correo") String correo) {
        Usuario_comun modelo = this.comunBO.obtenerUsuarioComunXCorreo(correo);
        if (modelo == null) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity(Map.of("error", "Usuario con correo: " + correo + ", no encontrado"))
                    .build();
        }
        return Response.ok(modelo).build();
    }
    ...
    @POST
    @Path("{id}/karma/{aumento}")
    public void actualizarKarma(@PathParam("id") int id, 
                               @PathParam("aumento") int aumento) {
        comunBO.actualizarKarma(id, aumento);
    }
}
