...
public class VotoPublicacionResource {
    ...
    @DELETE
    @Path("usuario/{idUsuario}/publicacion/{idPublicacion}")
    public Response eliminarVotoPublicacion(
        @PathParam("idUsuario") int idUsuario,
        @PathParam("idPublicacion") int idPublicacion
    ){
        try{
            this.votopubli.eliminarVotoPublicacion(idUsuario, idPublicacion);
            return Response.noContent().build();
        }catch(Exception e){
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(Map.of("error", "No se pudo eliminar el voto: " + e.getMessage()))
                .build();
        }
    }
}
