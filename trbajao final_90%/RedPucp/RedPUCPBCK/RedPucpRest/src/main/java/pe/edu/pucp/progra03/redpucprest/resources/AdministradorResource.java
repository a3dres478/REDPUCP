...
public class AdministradorResource {
    ...
    public boolean login(@FormParam("email") String email, 
        ...
        return this.adminBO.verificarLoginAdmin(email, password);
    }
    
    @GET
    @Path("correo/{correo}")
    public Response obtenerPorCorreo(@PathParam("correo") String correo) {
        Administrador modelo = this.adminBO.obtenerAdminXCorreo(correo);
        if (modelo == null) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity(Map.of("error", "Admin con correo: " + correo + ", no encontrado"))
                    .build();
        }
