...
public class VotoPublicacionDAOImpl extends TransaccionalBaseDAO<VotoPublicacion> implements VotoPublicacionDAO{
    ...
    @Override
    public void eliminarVotoPublicacion(int idUsuario,int idPublicacion){
        try{
            Connection conn = DBManager.getInstance().getConnection();
            PreparedStatement ps = this.comandoEliminarVotoPublicacion(conn, idUsuario, idPublicacion);
            ResultSet rs = ps.executeQuery();
        }catch(SQLException e){
            System.err.println("Error SQL durante la eliminacion: " + e.getMessage());
            throw new RuntimeException("No se pudo eliminar el voto.", e);    
        }catch(Exception e){
            System.err.println("Error inesperado: " + e.getMessage());
            throw new RuntimeException("Error inesperado al eliminar el voto.", e); 
        }
    }
    
    protected CallableStatement comandoEliminarVotoPublicacion(Connection conn,int idUsuario,int idPublicacion) 
        throws SQLException
    {
        String sql = "{CALL sp_eliminarVotoPublicacion(?,?)}";
        CallableStatement cmd = conn.prepareCall(sql);
