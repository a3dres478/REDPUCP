...
public class VotoComentarioDAOImpl extends TransaccionalBaseDAO<VotoComentario> implements VotoComentarioDAO{
    ...
    @Override
    public void eliminarVotoComentario(int idUsuario,int idComentario){
        try{
            Connection conn = DBManager.getInstance().getConnection();
            PreparedStatement ps = this.comandoEliminarVotoComentario(conn, idUsuario, idComentario);
            ResultSet rs = ps.executeQuery();
        }catch(SQLException e){
    ...
    protected CallableStatement comandoEliminarVotoComentario(Connection conn,int idUsuario,int idComentario) 
        throws SQLException
    {
